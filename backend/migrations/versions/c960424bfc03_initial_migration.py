...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "current_phase", sa.String(length=50), nullable=False, server_default="initialization"
        ),
        sa.Column(
    ...
    op.create_index(
        "idx_approval_requests_business_id", "approval_requests", ["business_id"], unique=False
    )
    op.create_index("idx_approval_requests_status", "approval_requests", ["status"], unique=False)
    op.create_index(
        "idx_metrics_business_id_recorded_at",
        "metrics_snapshots",
